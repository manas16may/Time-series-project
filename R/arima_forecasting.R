library(tseries)
library(forecast)
rm(list = ls()) # remove existing environmental variables
data = read.csv('data/train_data.csv') 
data$Date <- as.Date(data$Date) 
data1 = read.csv('data/test_data.csv')
...
tsdisplay(residuals(model.fit), lag.max = 10,
...
## using auto.arima
auto.arima(y_train$return,max.p=100,max.q=100,max.d=100)
